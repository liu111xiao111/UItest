# -*- coding: utf-8 -*-

from utility.device_info_util import DeviceInfoUtil

appVersion = '4.8.0.0'

phoneVersion = 'iPhone5c'

buildVersion = '9.3.2'

deviceID = DeviceInfoUtil().getUdid()

caseList = {
    'PinPaiJieTestCase'                     : u'品牌街',
    'QinZiTestCase'                         : u'亲子',
    'QuanChengSouSuoPinPai'                 : u'全城搜索(品牌)',
    'QuanChengSouSuoShangPin'               : u'全城搜索(商品)',
    'QuanChengSouSuoMenDian'                : u'全城搜索(门店)',
    'FeiFanTongZhangDanTestCase'            : u'飞凡通账单',
    'FeiFanTongQitaRukouTestCase'           : u'飞凡通其他快捷入口',
    'FeiFanTongShiminGongjiaokaTestCase'    : u'飞凡同市民/公交卡',
    'MeiShiHuiTestCase'                     : u'美食汇',
    'ReCiSousuoTestCase'                    : u'热词搜索',
    'MaiDanTestCase'                        : u'买单',
    'WoDeDengLuTestCase'                    : u'我的登录',
    'woDeTuiChuTestCase'                    : u'我的退出',
    'WoDeHuiYuanKaBaoTestCase'              : u'我的会员卡包',
    'WoDeXiaoXiZhongXinTestCase'            : u'我的消息中心',
    'DianYingTestCase'                      : u'电影',
    'WoDeXiHuanTestCase'                    : u'我的喜欢',
    'WoDeFeiFanTongTestCase'                : u'我的飞凡通',
    'ShouYeTingCheTestCase'                 : u'首页停车',
    'WoDeTingCheJiaoFeiTestCase'            : u'我的停车缴费',
    'WoDeGeRenXinXiTestCase'                : u'我的个人信息',
    'ShoppingMallCases'                     : u'购物中心',
    'GouWuZhongXinTestCase'                 : u'我的设置(小额免密设置)',
    'GuangChangZhaoDianTestCase'            : u'广场找店',
    'GuangChangMeiShiHui'                   : u'广场美食汇',
    'GuangChangMaiDan'                      : u'广场买单',
    'GuangChangTingChe'                     : u'广场停车',
    'GuangChangZiYuanWei'                   : u'广场资源位',
    'GuangChangSouSuo'                      : u'广场搜索',
    'AiGouWuTestCase'                       : u'广场爱购物',
    'WoDeSheZhiMiMaTestCase'                : u'我的设置(修改密码)',
    'BanBenShengJiTestCase'                 : u'版本升级',
    'YaoYiYaoTestCase'                      : u'摇一摇',
    'XianChangYaoTestCase'                  : u'广场现场摇',
    'WoDeDingDanTestCase'                   : u'我的订单',
    'HuiYuanTestCase'                       : u'广场会员',
    'PaiDuiQuHaoTestCase'                   : u'我的排队',
    'PiaoQuanTestCase'                      : u'我的票券',
    'ChenShiQieHuanTestCase'                : u'城市切换',
    'QianDaoTestCase'                       : u'签到',
    'ShangChaoTestCase'                     : u'商超',
    'MingPinYouDianTestCase'                : u'名店优品',
    'GuangChangXiangQingTestCase'           : u'广场详情',
    'YouHuiQuanTestCase'                    : u'广场优惠券',
    'PaiDuiTestCase'                        : u'广场排队取号',
    'SquareIndoorMapCases'                  : u'广场室内地图',
    'GuangChangDianYingGuangTestCase'       : u'广场电影逛',
    'HuiShengHuoRuKouTestCase'              : u'慧生活入口',
    'ShanPingShouYeTestCase'                : u"闪屏首页"
}
