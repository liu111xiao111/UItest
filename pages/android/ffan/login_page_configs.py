# -*- coding: utf-8 -*-


class LoginPageConfigs():
    # 普通登录手机号码输入框
    resource_id_user_name = "com.wanda.app.wanhui:id/edit_phone_number";

    # 普通登录密码输入框
    resource_id_pass_word = "com.wanda.app.wanhui:id/widget_password_input";

    # 登录按钮
    resource_id_login_button = "com.wanda.app.wanhui:id/btn_login";

    # text指明类型为text label,后面是文字的拼音
    text_quick_login = "快捷登录";
    text_normal_login = u"密码登录";
    text_forget_password = "忘记密码?"

    account_name = "13591822125"
    account_passwd = "Neu1234!"

    def __init__(self):
        pass;