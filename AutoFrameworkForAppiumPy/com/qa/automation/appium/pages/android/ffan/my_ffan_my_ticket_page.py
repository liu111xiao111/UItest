...
class MyFfanMyTicketPage(SuperPage):
    ...
    def validSelf(self):
        ...
        '''
        API().assert_view_by_resourceID_Until_android(testcase = self.testcase, driver = self.driver, logger = self.logger, resource_id = MyFfanMyTicketPageConfigs.resource_id_tv_my_ticket_tv)                             

    def clickOnTicketUnused(self):
        '''
        usage : Click "未使用" in my ticket page， and load "未使用" tab correctly. 
        ''' 
        API().click_view_by_text_android(testcase = self.testcase, driver=self.driver, logger = self.logger, text = MyFfanMyTicketPageConfigs.text_ticket_unused)

    def clickOnTicketUsed(self):
        '''
        usage : Click "已使用" in my ticket page， and load "已使用" tab correctly. 
        ''' 
        API().click_view_by_text_android(testcase = self.testcase, driver=self.driver, logger = self.logger, text = MyFfanMyTicketPageConfigs.text_ticket_used)

    def clickOnTicketOutOfDate(self):
        '''
        usage : Click "已过期" in my ticket page， and load "已过期" tab correctly. 
        ''' 
        API().click_view_by_text_android(testcase = self.testcase, driver=self.driver, logger = self.logger, text = MyFfanMyTicketPageConfigs.text_ticket_out_of_date)

    def clickOnReturnRefund(self):
        '''
        usage : Click "退货退款" in my ticket page， and load "退货退款" tab correctly. 
        ''' 
        API().click_view_by_text_android(testcase = self.testcase, driver=self.driver, logger = self.logger, text = MyFfanMyTicketPageConfigs.text_ticket_return_refund)
